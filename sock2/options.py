...
what they mean and what their types are, defined pythonic type-converter for
all the types, and wrote down huge tables of
...
def LingerOption(level, option, doc):
    ...
    @option_error_decorator
    def getter(self):
        active, value = struct.unpack(format, self._sock.getsockopt(level,
            option, size))
        if active:
            return value
        else:
    ...
    def setter(self, value):
        if value is None:
            ...
            active = 1
        return self._sock.setsockopt(level, option, struct.pack(format, active,
            value))
    return property(getter, setter, doc = doc)
...
def _bsd_timeval_option(level, option, doc):
    ...
    @option_error_decorator
    def getter(self):
        sec, usec = struct.unpack(format, self._sock.getsockopt(level, option,
            size))
        return sec + usec / 1e6
    @option_error_decorator
    def setter(self, value):
        ...
        return self._sock.setsockopt(level, option, struct.pack(format, sec,
            usec))
    return property(getter, setter, doc = doc)
    ...
    def class_creator(name, bases, namespace):
        for (optname, propname, proptype, doc) in options:
            ...
            if hasattr(level_namespace, optname):
                namespace[propname] = proptype(socklevel,
                    getattr(level_namespace, optname), doc)
        return type(name, bases, namespace)
    return class_creator


class SocketLevelOptions(object):
    __metaclass__ = LevelOptions(consts.OptionLevels.SOCKET,
        consts.SocketLevelOptions, socket_level_options)
    __slots__ = []


class IpLevelMixin(object):
    __metaclass__ = LevelOptions(consts.OptionLevels.IP, consts.IpLevelOptions,
        ip_level_options)
    __slots__ = []

    def join_group(self, (mcast, iface)):
        """joins the given multicast group. takes a tuple of
        (ipaddr-string, ipaddr-string)"""
        self._add_membership = (mcast, iface)

    def leave_group(self, (mcast, iface)):
        """leaves the given multicast group. takes a tuple of
        (ipaddr-string, ipaddr-string)"""
...
class Ipv6LevelMixin(object):
    ...
    def join_group(self, (mcast, iface)):
        """joins the given multicast group. takes a tuple of
        (ipv6addr-string, interface-number)"""
...
class TcpLevelMixin(object):
    __metaclass__ = LevelOptions(consts.OptionLevels.TCP,
        consts.TcpLevelOptions, tcp_level_options)
    __slots__ = []
